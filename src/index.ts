...
// Tools: post, browse, check_notifications, reply, find_submolt, goto_submolt
// Added: URLs in browse output for human reference
...
async function executeMoltbookBrowse(
 ...
 if (response.ok) {
 const data = await response.json() as { posts?: { id?: string; title?: string; author?: { name?: string }; upvotes?: number; comment_count?: number; submolt?: { name?: string } }[] };
 ...
 const postUrl = id ? `${MOLTBOOK_WEB}/post/${id}` : "";

 report += `[${submoltName}] ${title}\n`;
 report += `  by ${author} | ${upvotes}↑ ${comments}💬\n`;
 if (postUrl) {
 report += `  → ${postUrl}\n`;
 }
 report += `\n`;
...
async function browseWebFallback(submolt: string, limit: number): Promise<{ content: any[] }> {
 ...
 report += `[${submolt}] ${title}\n`;
 report += `  by ${author} | ${upvotes}↑\n`;
 report += `  → ${MOLTBOOK_WEB}/m/${submolt}\n`;
 report += `\n`;
 count++;
 }
 }
 ...
 report += "(Posts found but details incomplete - parsing limited)\n\n";
 for (const t of anyTitles.slice(0, limit)) {
 const cleanTitle = t.replace(/<[^>]+>/g, "").trim();
 if (cleanTitle.length > 10) {
 report += `- ${cleanTitle.substring(0, 100)}...\n`;
 }
 }
 report += `\nView full submolt: ${MOLTBOOK_WEB}/m/${submolt}\n`;
 } else {
 report += "(No parseable posts found - submolt may be empty or requires login)\n";
 }
 }
...
const moltbookPlugin = {
 ...
 description: "Browse moltbook feed for posts and engagement opportunities. Returns URLs for each post.",
 parameters: {
 type: "object",
 properties: {
